    ...
    public function registrar()
    {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if (!preg_match('/^\d{8}$/', $dni)) {
                $error_message .= "El DNI debe tener exactamente 8 dígitos.<br>";
            }
            ...
            if (!preg_match('/^[A-Z\s]{3,30}$/', $nombre)) {
                $error_message .= "El nombre debe tener entre 3 y 30 caracteres y solo letras mayúsculas, espacios.<br>";
            }
            if (!preg_match('/^[A-Z\s]{5,30}$/', $apellido)) {
                $error_message .= "El apellido debe tener entre 5 y 30 caracteres y solo letras mayúsculas, espacios.<br>";
            }

            // Validación del usuario
            if (!preg_match('/^[a-zA-Z0-9]{5,16}$/', $usuario)) {
                $error_message .= "El nombre de usuario debe tener entre 5 y 16 caracteres y solo letras y números.<br>";
            }

            // Validación de la contraseña
            if (strlen($password) < 6 || strlen($password) > 20) {
                $error_message .= "La contraseña debe tener entre 6 y 20 caracteres.<br>";
            }
            ...
            if (!empty($buscar_usuario)) {
                $error_message .= "El nombre de usuario ya está en uso.<br>";
            }
